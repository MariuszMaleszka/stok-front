...
const isFormValid = computed(() => {
  if (viewStore.stepOne === viewStore.STEP_ONE_DATA) {
    return stayStore.dateOfStay && stayStore.canProceed
  } else if (viewStore.stepOne === viewStore.STEP_ONE_PREFERENCES) {
    return stayStore.participants.every(p => {
      console.log('Participant:', p) // Debug log
      if (!p.name?.trim()) return false
      if (!p.activityType) return false // Check if this property name is correct
      if (p.participantType === 'child' && (p.age === null || p.age < 4 || p.age > 16)) return false
      if (!p.skillLevel) return false
      return true
    })
  }
  return false
